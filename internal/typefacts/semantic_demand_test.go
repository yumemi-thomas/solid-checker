...
func TestRetainedContributionSharesCanonicalEntityBacking(t *testing.T) {
	...
	if contribution == nil || len(contribution.entities) != 1 || len(table.entityRuns) != 1 || len(table.entityRuns[0].entities) != 1 {
		t.Fatalf(
			"unexpected retained state: contribution=%+v entities=%d",
			contribution,
			len(table.entityRuns),
		)
	}
	if &contribution.entities[0] != &table.entityRuns[0].entities[0] {
		t.Fatal("transport entity run copied the retained contribution")
	}
}
