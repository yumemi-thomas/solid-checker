...
func demandListHash(demands []EntityDemand, seed maphash.Seed) uint64 {
	...
	for index := range demands {
		...
		buffer = append(buffer,
			...
			flag(demand.Callability), flag(demand.RuntimeValueDomain), flag(demand.CallResultDomain), flag(demand.ConstantValue), flag(demand.References), flag(demand.Async), flag(demand.StructuralAccessor),
