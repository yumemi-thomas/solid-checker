...
- Started: 2026-08-26T00:01:49.166Z
- Finished: 2026-08-26T00:11:45.338Z
- Manifest generated at: 2026-08-22T07:44:17.857Z (rows: 305, probes: 416)
- Probe rows run: 416
- Checker native binary: `c1b606862f4ea98ac719c0d53c1db51d19a0f62e60ffb9f2899bb8b85d0f6cf8` (73908480 bytes, mtime 2026-08-25T23:59:29.896Z)
...
| --- | --- |
| Probe rows run | 416 |
| Reached a generated contract | 396/416 (95.19%) |
| **Reached `verified`** | **297/416 (71.39%)** of all rows |
| Reached `verified`, of rows that produced a contract | 297/396 (75.00%) |
| Refused by `contract verify` | 97/416 (23.32%) |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Official Solid | 23 | 23 | 13/23 (56.52%) | 8 | 1668/2511 (66.43%) | 1668/1668 (100.00%) | 165 | 591 | 325 |
| Kobalte | 6 | 3 | 1/6 (16.67%) | 2 | 1148/1744 (65.83%) | 1148/1148 (100.00%) | 2 | 7 | 3 |
| Solid Primitives | 289 | 280 | 229/289 (79.24%) | 51 | 1921/3316 (57.93%) | 1921/1921 (100.00%) | 485 | 598 | 823 |
| Corvu | 28 | 28 | 18/28 (64.29%) | 10 | 289/427 (67.68%) | 289/289 (100.00%) | 36 | 60 | 60 |
| TanStack | 52 | 48 | 28/52 (53.85%) | 20 | 2128/2779 (76.57%) | 2128/2128 (100.00%) | 109 | 552 | 397 |
| Solid Devtools | 12 | 9 | 6/12 (50.00%) | 3 | 126/271 (46.49%) | 126/126 (100.00%) | 5 | 29 | 18 |
...
| --- | --- | --- | --- | --- |
| solid1 | 168 | 152 | 115/168 (68.45%) | 37 |
| solid2 | 248 | 244 | 182/248 (73.39%) | 60 |

## Why verification refuses

97 rows were refused. `contract verify` raises every blocker it finds rather than stopping at the first, so the row counts below sum to more than the number of refused rows.

| Blocker (RFC 0002 §3) | Rows raising it | Blocker lines |
| --- | --- | --- |
| `kind-observed` | 62 | 134 |
| `probe-report-includes-evidence-write` | 31 | 31 |
| `incompleteness` | 31 | 518 |
...
| Figure | Count |
| --- | --- |
| Claims planned across every probed contract | 11744 |
| Driven | 7734/11744 (65.85%) |
| Passed | 7734/11744 (65.85%) |
| Failed | 0 |
| Undriven | 4010/11744 (34.15%) |
| Incompleteness findings | 518 |

Undriven claims by reason:

| Reason | Claims |
| --- | --- |
| no probe form: reactiveReads | 1079 |
| no plantable reactive source | 859 |
| entrypoint import threw | 601 |
| no probe form: ownerRequirements | 463 |
| synthesized call threw | 368 |
| synthesized call did not invoke the callback | 286 |
| no probe form: asyncBehavior | 98 |
| no unambiguous summary for the mode | 59 |
| no probe form: store path | 51 |
| probe session wrote no report | 36 |
| probe session aborted by package code | 33 |
| runtime re-runs nothing in this mode | 33 |
| callback ran more often than the call site | 25 |
| no probe form: callback arguments | 9 |
| planted write was never re-read | 6 |
| callback re-ran with nothing written | 2 |
| probe session hit the per-mode timeout | 2 |
...
- `kind` obligations with at least one gapped stated mode: 2269

| Why the mode produced no passing `kind` observation | (claim, mode) pairs |
| --- | --- |
| entrypoint import threw | 3819 |
| probe session wrote no report | 120 |
| probe session aborted by package code | 96 |
| no unambiguous summary resolves in the mode (no kind claim exists) | 59 |
| export-missing in this mode | 45 |
| probe session hit the per-mode timeout | 9 |
...
| Error [ERR_PACKAGE_PATH_NOT_EXPORTED]: Package subpath './web' is not defined by "exports" in <path> | 83 |
| Error [ERR_MODULE_NOT_FOUND]: Cannot find package '@solid-primitives/utils' imported from /private/t | 82 |
...
- Rows where at least one session faked at least one global: 391

| Faked global | Rows |
| --- | --- |
| `IntersectionObserver` | 391 |
| `MutationObserver` | 391 |
| `ResizeObserver` | 391 |
| `cancelAnimationFrame` | 391 |
| `document` | 391 |
| `getComputedStyle` | 391 |
| `history` | 391 |
| `localStorage` | 391 |
| `location` | 391 |
| `matchMedia` | 391 |
| `requestAnimationFrame` | 391 |
| `screen` | 391 |
| `self` | 391 |
| `sessionStorage` | 391 |
| `window` | 391 |
...
| Figure | Count |
| --- | --- |
| Worker processes started | 18834 |
| Of those, restarts after a throw | 15609 |
| Sessions that died (crash, timeout, unreadable output) | 66 |
...
| Exports carrying an unknown in the verified rows, at generation | 1696/4829 (35.12%) |
| Exports carrying an unknown in the verified rows, after verification | 1955/3793 (51.54%) |

How much a verified contract actually certifies from observation:

| Figure | Count |
| --- | --- |
| Verified rows carrying at least one probed behavioral row | 21/297 (7.07%) |
| Probed behavioral row markers kept across the whole corpus | 97 |
| Inferred row markers dropped by verification | 4514 |
| Probed markers discarded as unwitnessed by this run's report | 151 |
| Entrypoints verification refused inside a promoted document | 52 |
| Verified rows carrying at least one such refusal | 15 |
...
| State | Exports |
| --- | --- |
| (a) certified by a verified contract | 1838/8124 (22.62%) |
| (b) honest unknown inside a verified contract | 1955/8124 (24.06%) |
| (c) dropped from a verified contract with its refused entrypoint | 1036/8124 (12.75%) |
| (d) inside a contract that never reached `verified` | 3295/8124 (40.56%) |
...
| --- | --- | --- | --- | --- | --- |
| install | 416 | 829 ms | 1930 ms | 18794 ms | 1067 ms |
| generate | 413 | 184 ms | 2116 ms | 120075 ms | 2320 ms |
| probe | 394 | 637 ms | 3423 ms | 226345 ms | 3595 ms |
| verify | 394 | 53 ms | 79 ms | 176 ms | 59 ms |
| pipelineWithoutInstall | 413 | 1001 ms | 5933 ms | 291830 ms | 5806 ms |
| total | 416 | 1975 ms | 10021 ms | 294676 ms | 6881 ms |
...
| `@tanstack/solid-query-persist-client@5.101.4|solid1|only` | tanstack | `kind-observed` | 2 | kind-observed | probe session aborted by package code x3 |
...
| `@solid-devtools/shared@0.20.0|solid1|only` | 21 | 8 | 5 | 0 | `./chunk-DTKGRNV6`, `./detect`, `./utils` |
...
| `@solidjs/web@2.0.0-rc.1|solid2|floor` | 318 | 65 | 47 | 7 | `.`, `./frames`, `./server-functions` |
...
| `@tanstack/devtools-utils@0.7.0|solid1|only` | 7 | 3 | 0 | 0 | `./preact`, `./react`, `./svelte`, `./vue` |
...
| `@tanstack/solid-query@5.101.4|solid1|only` | 57 | 40 | 17 | 4 | — |
...
| `@tanstack/solid-table@9.1.2|solid1|only` | 295 | 14 | 1 | 0 | `.` |
