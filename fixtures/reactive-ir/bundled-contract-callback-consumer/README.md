...
`createMemo` and `createSignal` exactly, and `tsc --noEmit` on this project is
clean: a declaration cannot say when a callback runs, which is the whole reason
the contract carries the row.
