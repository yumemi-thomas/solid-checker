...
// discovered accessor.
function createPair<T>(initial: Exclude<T, Function>): [Accessor<T>, Setter<T>] {
  const [value, setValue] = createSignal(initial);
  return [value, setValue];
}

function createNestedPair<T>(initial: Exclude<T, Function>): [[Accessor<T>, Setter<T>]] {
...
export function literalIndex<T>(initial: Exclude<T, Function>): Accessor<T> {
...
export function wrappedLiteralIndex<T>(initial: Exclude<T, Function>): Accessor<T> {
...
export function wholeBinding<T>(initial: Exclude<T, Function>): [Accessor<T>, Setter<T>] {
...
export function splitVarDeclaration<T>(initial: Exclude<T, Function>): [Accessor<T>, Setter<T>] {
...
export function staticProperty<T>(initial: Exclude<T, Function>): Accessor<T> {
...
export function arraySlot<T>(initial: Exclude<T, Function>): Accessor<T> {
...
export function arraySlotSetter<T>(initial: Exclude<T, Function>): Setter<T> {
...
export function computedIndex<T>(initial: Exclude<T, Function>, at: 0 | 1): Accessor<T> | Setter<T> {
...
// whole object.
export function wrappedQuotedMember<T>(initial: Exclude<T, Function>): Accessor<T> {
...
export function wrappedUnknownProperty<T>(initial: Exclude<T, Function>): number {
...
export function atZero<T>(initial: Exclude<T, Function>): Accessor<T> | Setter<T> | undefined {
...
export function defaultedSlot<T>(initial: Exclude<T, Function>, fallback: Accessor<T>): Accessor<T> {
...
export function nestedSlot<T>(initial: Exclude<T, Function>): Accessor<T> {
  const [[value]] = createNestedPair(initial);
  return value;
}

// A rest element binds a slice of the tuple, not an item of it.
export function restSlot<T>(initial: Exclude<T, Function>): [Setter<T>] {
  const [, ...rest] = createPair(initial);
  return rest;
}

// An object pattern binds a property of the result, not the result.
export function objectSlot<T>(initial: Exclude<T, Function>): Accessor<T> {
