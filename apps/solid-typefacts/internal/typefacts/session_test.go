...
func TestLatestSemanticDomainsAreAvailableInV1(t *testing.T) {
	...
	if got := decodeTransitionEnvelopeForTest(t, response.TableTransition).schema; got != TypeFactsTableSchemaVersionV18 {
		t.Fatalf("active Wire table schema = %d, want %d", got, TypeFactsTableSchemaVersionV18)
	}
