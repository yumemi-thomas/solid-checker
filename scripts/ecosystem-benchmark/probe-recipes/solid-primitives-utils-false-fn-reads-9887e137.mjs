...
//
// Demand-scoped: consumer projects import `falseFn` from this exact version
// (`docs/package-contract-v2/phase21/2026-09-12-consumer-demand-measurement.md`).
//
// `falseFn` is `() => false`. It owns no source. The samples pass an owned
...
import { falseFn } from "@solid-primitives/utils";
...
export async function runProbeSession(_session, harness) {
  ...
  for (const args of [[], [ownedByTheRecipe], [() => ownedByTheRecipe.current]]) {
    ...
    const answered = falseFn(...args);
    if (answered !== false) {
      throw new Error(`falseFn() answered ${String(answered)}`);
    }
    harness.emit({ marker: "call", kind: "call", phase: "exit" });
  }
  if (observed !== 0) {
    throw new Error(`falseFn read a caller-owned source ${observed} time(s)`);
  }
}
