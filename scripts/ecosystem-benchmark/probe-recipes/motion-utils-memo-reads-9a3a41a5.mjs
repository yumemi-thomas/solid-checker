...
//
// `memo(callback)` returns a getter that calls the caller's callback once and
// caches its answer in a closure variable. The callback is the caller's code
// (ADR 0034); the recipe's callback counts its own runs.
...
export async function runProbeSession(_session, harness) {
  ...
  expect("memo calls through once", cached() === 42 && cached() === 42 && runs === 1);
