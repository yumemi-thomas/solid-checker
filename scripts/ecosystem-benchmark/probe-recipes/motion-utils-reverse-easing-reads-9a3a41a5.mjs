...
//
// `reverseEasing(easing)` returns `p => 1 - easing(1 - p)` over the caller's
// easing (ADR 0034).
import { reverseEasing } from "motion-utils";
...
export async function runProbeSession(_session, harness) {
  ...
  const reversed = reverseEasing(p => p * p);
  expect("reverseEasing(p²)(0.5)", near(reversed(0.5), 0.75));
  expect("reverseEasing keeps the endpoints", reversed(0) === 0 && reversed(1) === 1);
