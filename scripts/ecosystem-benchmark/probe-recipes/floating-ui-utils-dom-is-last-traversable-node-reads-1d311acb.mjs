...
//
// NEVER EMITS: `read-operation`. The export reads `nodeName` off the node its caller supplied and matches it against a literal pattern (ADR 0034).
...
export async function runProbeSession(_session, harness) {
  ...
  // Without a `window` the node-name lookup takes the mocked-node fallback and
  // reports `#document`, which this pattern matches.
  expect(isLastTraversableNode({ nodeName: "DIV" }) === true, "the #document fallback matches");
