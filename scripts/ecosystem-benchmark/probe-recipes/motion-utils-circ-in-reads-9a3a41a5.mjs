...
//
// `circIn(p)` is `1 - sin(acos(p))`: two `Math` calls over one number.
import { circIn } from "motion-utils";
...
export async function runProbeSession(_session, harness) {
  ...
  expect("circIn(0)", near(circIn(0), 0));
  expect("circIn(1)", near(circIn(1), 1));
  expect("circIn(0.5) is inside the unit interval", circIn(0.5) > 0 && circIn(0.5) < 1);
