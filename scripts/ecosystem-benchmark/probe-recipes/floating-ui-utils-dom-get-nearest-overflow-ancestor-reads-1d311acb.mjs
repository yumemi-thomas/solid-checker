...
//
// NEVER EMITS: `read-operation`. The export walks parent links and reads `ownerDocument.body` off the node its caller supplied, which is the caller's own object (ADR 0034).
...
export async function runProbeSession(_session, harness) {
  ...
  const body = { tag: "body" };
  const node = { parentNode: { nodeName: "DIV" }, ownerDocument: { body } };
  expect(getNearestOverflowAncestor(node) === body, "a traversable parent yields the document body");
