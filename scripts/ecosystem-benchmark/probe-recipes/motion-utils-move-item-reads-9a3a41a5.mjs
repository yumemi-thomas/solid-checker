...
//
// `moveItem([...arr], from, to)` copies the caller's array on entry and
// splices the copy. Every read is of the caller's array (ADR 0034).
...
export async function runProbeSession(_session, harness) {
  ...
  const list = [1, 2, 3];
  expect("moveItem([1, 2, 3], 0, 2)", same(moveItem(list, 0, 2), [2, 3, 1]));
  expect("moveItem leaves the caller's array alone", same(list, [1, 2, 3]));
  expect("moveItem with a negative index counts from the end", same(moveItem([1, 2, 3], -1, 0), [3, 1, 2]));
  expect("moveItem out of range returns the copy unchanged", same(moveItem([1, 2, 3], 5, 0), [1, 2, 3]));
