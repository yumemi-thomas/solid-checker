...
//
// `getSideAxis(placement)` reads `placement[0]` and compares it with `"t"` and
// `"b"`: one index read of the caller's string.
...
export async function runProbeSession(_session, harness) {
  for (const [placement, expected] of [
    ...
    const answered = getSideAxis(placement);
    if (answered !== expected) {
      throw new Error(`getSideAxis(${placement}) answered ${String(answered)}`);
    }
