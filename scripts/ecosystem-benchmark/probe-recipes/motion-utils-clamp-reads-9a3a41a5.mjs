...
//
// `clamp(min, max, v)` is two comparisons over three numbers.
...
export async function runProbeSession(_session, harness) {
  ...
  for (const [min, max, v, expected] of [[0, 10, 5, 5], [0, 10, -1, 0], [0, 10, 15, 10], [3, 3, 3, 3]]) {
    expect(`clamp(${min}, ${max}, ${v})`, Object.is(clamp(min, max, v), expected));
  }
