...
//
// `isNumericalString(v)` is one regular-expression test over the caller's
// string.
...
export async function runProbeSession(_session, harness) {
  ...
  for (const [value, expected] of [["12.5", true], ["-3", true], [".5", true], ["1px", false], ["", false]]) {
    expect(`isNumericalString(${JSON.stringify(value)})`, isNumericalString(value) === expected);
  }
