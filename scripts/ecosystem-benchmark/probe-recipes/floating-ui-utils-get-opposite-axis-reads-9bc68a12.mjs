...
//
// `getOppositeAxis(axis)` is `axis === "x" ? "y" : "x"`: one strict comparison
...
import { getOppositeAxis } from "@floating-ui/utils";

export async function runProbeSession(_session, harness) {
  for (const [axis, expected] of [["x", "y"], ["y", "x"], ["z", "x"]]) {
    harness.emit({ marker: "call", kind: "call", phase: "enter" });
    const answered = getOppositeAxis(axis);
    if (answered !== expected) {
      throw new Error(`getOppositeAxis(${axis}) answered ${String(answered)}`);
    }
