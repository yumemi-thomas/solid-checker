...
// implementation census refused `createHydratableSignal` outright: its only
// uncensused form is `sharedConfig.context`, whose receiver is imported from
...
// options)` and stops. On the client it reads `sharedConfig.context`, and
// either seeds a signal and defers `update()` through `onMount`, or calls
...
import { createHydratableSignal, isServer } from "@solid-primitives/utils";
...
export async function runProbeSession(_session, harness) {
  ...
  for (let sample = 0; sample < 3; sample += 1) {
    ...
    const answered = createHydratableSignal("from-the-server", () => {
      updateCalls += 1;
    ...
    if (!Array.isArray(answered) || answered.length !== 2 ||
      ...
      throw new Error("createHydratableSignal did not answer a [getter, setter] pair");
    }
  }
