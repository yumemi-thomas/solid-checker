...
//
// `warnOnce(condition, message)` returns early when the condition holds,
// before it reads or writes the module's `Set` of warned messages. Only that
// branch is sampled, so nothing is logged and the set is untouched.
import { warnOnce, hasWarned } from "motion-utils";
...
export async function runProbeSession(_session, harness) {
  ...
  expect("warnOnce(true) returns undefined", warnOnce(true, "probe recipe", "probe") === undefined);
  expect("warnOnce(true) records nothing", hasWarned("probe recipe") === false);
