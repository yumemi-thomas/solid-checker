...
//
// `pipe(...transformers)` folds the caller's functions into one. Every call
// is into the caller's code (ADR 0034).
...
export async function runProbeSession(_session, harness) {
  ...
  expect("pipe(x + 1, x * 2)(3)", pipe(x => x + 1, x => x * 2)(3) === 8);
  const single = x => x - 1;
  expect("pipe of one function is that function", pipe(single) === single);
