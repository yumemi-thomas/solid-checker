...
//
// `invariant(check, message)` throws when `check` is false and otherwise
// returns `undefined`, in either build of the module. Only the passing branch
// is sampled, so no error object is built and nothing is logged.
...
export async function runProbeSession(_session, harness) {
  ...
  expect("invariant(true) returns undefined", invariant(true, "probe recipe", "probe") === undefined);
