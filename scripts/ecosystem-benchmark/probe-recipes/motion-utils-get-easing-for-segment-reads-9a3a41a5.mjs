...
//
// `getEasingForSegment(easing, i)` indexes the caller's array with `wrap`, or
// returns a non-array easing as it is. The only read is of the caller's array
// (ADR 0034).
import { getEasingForSegment } from "motion-utils";
...
export async function runProbeSession(_session, harness) {
  ...
  const first = p => p, second = p => p * p;
  expect("getEasingForSegment picks by wrapped index", getEasingForSegment([first, second], 3) === second);
  expect("getEasingForSegment returns a lone easing", getEasingForSegment(first, 7) === first);
