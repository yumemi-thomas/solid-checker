...
//
// `parseColor(value)` tries each colour class's parser over the caller's
// string and throws on failure. Regular expressions and constructors of
// objects the package builds; no reactive source.
...
export async function runProbeSession(_session, harness) {
  ...
  expect("parseColor('#ff0000') is red", red.getChannelValue("red") === 255 && red.getChannelValue("blue") === 0);
  const rgb = parseColor("rgb(1, 2, 3)");
  expect("parseColor('rgb(1, 2, 3)') keeps the channels", rgb.getChannelValue("green") === 2);
  const hsl = parseColor("hsl(120, 50%, 50%)");
  expect("parseColor('hsl(120, 50%, 50%)') keeps the hue", hsl.getChannelValue("hue") === 120);
  let threw = false;
  try {
    parseColor("nope");
  } catch {
  ...
  expect("parseColor throws on an invalid string", threw);
