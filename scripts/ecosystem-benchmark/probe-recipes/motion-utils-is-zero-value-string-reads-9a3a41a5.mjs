...
//
// `isZeroValueString(v)` is one regular-expression test over the caller's
// string.
...
export async function runProbeSession(_session, harness) {
  ...
  for (const [value, expected] of [["0px", true], ["0%", true], ["0", false], ["0.5", false], ["10px", false]]) {
    expect(`isZeroValueString(${JSON.stringify(value)})`, isZeroValueString(value) === expected);
  }
