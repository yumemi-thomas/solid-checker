...
//
// NEVER EMITS: `read-operation`. The export reads `parent` and `frameElement` off the window its caller supplied, which is the caller's own object (ADR 0034).
...
export async function runProbeSession(_session, harness) {
  ...
  expect(getFrameElement({ parent: {}, frameElement: frame }) === frame, "a framed window yields its frameElement");
  expect(getFrameElement({ parent: null }) === null, "no parent yields null");
  expect(
    getFrameElement({ parent: Object.create(null), frameElement: frame }) === null,
    "a prototype-less parent yields null"
  );
