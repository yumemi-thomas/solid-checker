...
//
// `normalizeColor(v)` parses a string and returns any other value as it is.
// The only read is `typeof` on the caller's argument (ADR 0034).
import { normalizeColor } from "@solid-primitives/utils/colors";
...
export async function runProbeSession(_session, harness) {
  ...
  const parsed = normalizeColor("#ff0000");
  expect("normalizeColor parses a string", parsed.getChannelValue("red") === 255 && parsed.getChannelValue("green") === 0);
  const already = normalizeColor("rgb(0, 0, 255)");
  expect("normalizeColor returns a non-string as it is", normalizeColor(already) === already);
