...
//
// `isBezierDefinition(easing)` is `Array.isArray` and one `typeof` on the
// caller's value (ADR 0034).
...
export async function runProbeSession(_session, harness) {
  ...
  expect("four numbers are a bezier definition", isBezierDefinition([0.42, 0, 1, 1]) === true);
  expect("an array of names is not", isBezierDefinition(["easeIn"]) === false);
  expect("a string is not", isBezierDefinition("easeIn") === false);
