...
impl Connection {
    fn spawn(producer: &Producer, project_id: &str) -> Result<Self, SessionError> {
        ...
        let mut child = command
            .args(["-schema=7", "-project", project_id])
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .spawn()
        ...
        let expected = (
            ...
            v3::TYPE_FACTS_SCHEMA_V7_SHA256,
...
fn request(operation: Operation, project_id: &str, generation: u64) -> Request {
    Request {
        schema: v3::TYPE_FACTS_SCHEMA_V7,
        request_id: 0,
        operation,
...
fn prepare_analyze_response(
    ...
    if response.schema != v3::TYPE_FACTS_SCHEMA_V7 {
        return Err(SessionError::InvalidResponse(format!(
            "response schema is {}, expected {}",
            response.schema,
            v3::TYPE_FACTS_SCHEMA_V7
        )));
    }
    ...
    if response.table_transition.is_empty() {
        ...
        if candidate.schema() != v3::TYPE_FACTS_TABLE_SCHEMA_V4
    ...
    if transition.project_id.as_ref() != response.project_id.as_str()
        ...
        || transition.table_schema != v3::TYPE_FACTS_TABLE_SCHEMA_V4
    {
        return Err(SessionError::InvalidResponse(
...
mod tests {
    ...
    fn delta_transition(
        ...
        WireTableTransition {
            ...
            table_schema: v3::TYPE_FACTS_TABLE_SCHEMA_V4,
            base_generation,
            target_generation,
