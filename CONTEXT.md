...
Since ADR 0043 the **root set** itself is closed under the reads this census dispositions: a name a parameter's own object binding pattern bound, and a name a local declaration bound from an already-rooted initializer (to a fixpoint), are rooted, because naming an intermediate does not change whose value it is. Every stated subject carries `subjectRoot`, the derivation that rooted it, over a closed set — `parameter` for all of the above, and `parameter-default` for the one different claim, a parameter whose default names another rooted parameter, whose value is caller-supplied under either of two branches. An unreviewed or absent derivation refuses. ADR 0050 widens `parameter` to a binding the file **writes**, when every value it can hold is rooted at one slot — a join over the binding's sources, not a walk over its control flow — and follows a conditional and the short-circuit operators the same way. ADR 0048 adds `parameter-result`, the value a call to a caller-supplied callee handed back — the caller's by ADR 0042's argument about what its iterable yielded. ADR 0047 asks the same question of `instanceof`, whose subject is its **right** operand and whose whole reach is that constructor's `Symbol.hasInstance`: `default-library` (the engine's own constructor) and `own-class` (a class this artifact declares with no heritage clause and no computed member) join the two parameter derivations, and each derivation carries exactly one companion fact — an index for the parameter ones, a declaration for `own-literal` and `own-class`, neither for `default-library`.
...
Since ADR 0046 an entry may also carry a `spelling` — `import("<specifier>").<Name>` — because the helper's own module is JavaScript and a bare `Axis` resolves to nothing there; it is written into the annotation, echoed byte for byte, and is never part of the falsifier, which stays the printed text and the identity.
_Avoid_: premise inheritance, propagated types, inferred helper signature, union of callers (each call site is its own condition), treating the spelling as the premise
