...
const baseline = 'rust/target/ecosystem-investigations/2026-09-08-original-helper-full.json';
const read = p => JSON.parse(readFileSync(p));
const hash = p => 'sha256:' + createHash('sha256').update(readFileSync(p)).digest('hex');
const scoped = new Map();
const startEvidence = 'docs/package-contract-v2/phase21/2026-09-09-inert-initialization-scoped-measurement.json';
for (const row of read(startEvidence).rows) scoped.set(row.probeId, { catalog: join(dirname(row.combinedAudit), 'catalog'), evidence: startEvidence, audit: row.combinedAudit });
const devtoolsEvidence = 'docs/package-contract-v2/phase21/2026-09-09-declaration-reexport-devtools-scoped-measurement.json';
scoped.set('solid-devtools@0.34.5|solid1|only', { catalog: '/private/tmp/inert-entrypoints-nwiTUG/catalog', evidence: devtoolsEvidence, audit: '/private/tmp/inert-entrypoints-nwiTUG/audit.json' });
...
writeFileSync('docs/package-contract-v2/phase21/2026-09-09-partial-row-audit.json', JSON.stringify(output, null, 2) + '\n');
