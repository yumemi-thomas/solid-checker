...
| every other `NODE_*` / arbitrary variable (`NODE_COMPILE_CACHE`, `NODE_REPL_EXTERNAL_MODULE`, …) | the environment | **REFUSED** — the allowlist is `HOME`, `TMPDIR`, `LANG`, `LC_ALL`, an emptied `NODE_OPTIONS`, and `SOLID_CHECKER_PROBE_NONCE`. `PATH` is deliberately absent. Since protocol v7 the recipe module is *not* an environment value: it is per session and travels in the session frame, so a worker may be booted before its session is chosen |
