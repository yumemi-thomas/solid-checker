...
**Subpath precision.** The bundled 1.x contract
(`pkg/contracts/bundled/solid-v1/solid-js.json`, generated by
`scripts/generate-bundled-solid1-contract.mjs`) keys its `entrypoints` map by
subpath — `.`, `./store`, `./web` — and the emitted sets are exact: an export
