...
- Purpose: the source `crates/solid-dialect/src/solid_1x.rs` cites for its
  vocabulary, the reference for every fixture `solid-js.d.ts` stub, and the
  source of rule documentation examples

> **Brought forward from the `1.x` branch.** `Solid1x`'s module doc has cited
> this file since the dialect crate was written, and until now the file existed
> only on that branch — a citation to nothing. The extraction is unchanged. The
...
> the vocabulary is a real export of a module the dialect owns. This file
> governs what may enter the vocabulary; the index governs where it lives.
...
**Subpath precision.** The bundled contract is an array of wire-version-2
units whose `scope` carries a `moduleSubpath`, and the emitted sets are exact:
an export appears only under the subpaths whose declaration entry provides it.
`import { createStore } from "solid-js"` is therefore a contract miss under
`.` while the name is covered under `./store`, and the checker reports it as
`package-contract-subpath-mismatch` (SC9010) instead of silently analyzing it
through name-keyed native semantics. The 2.0-era contract could not make this
distinction — it listed `createStore` under `solid-js`, and the pre-v2 shape
had no `moduleSubpath` at all, so its export set was the union of all three
subpaths. See `scripts/generate-bundled-solid-contract.mjs`.
